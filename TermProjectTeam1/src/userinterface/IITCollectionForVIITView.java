...
//       		    	myModel.stateChangeRequest("CancelInventoryItemTypeList", null); 
	...
	protected void addButtonsForSelected()
	{
		...
		{
			if(btnContainer.getChildren().contains(selectButton))
			{
				btnContainer.getChildren().remove(selectButton);
			}
//			System.out.println(selectedItem.getItemTypeName());
			String selectedIITName = selectedItem.getItemTypeName();
			selectButton = new Button("Select");
			selectButton.setOnAction(e -> {
				myModel.stateChangeRequest("SelectIIT", selectedIITName);
			});
			
			btnContainer.getChildren().addAll(selectButton);
